...
Hub::postRequest = (targetUrl, params, callback) ->
...
Hub::distribute = (activity, topic, callback) ->
  hub = this
...
Hub::makeMessage = (activity, topic) ->
  message = items: [
    topic: topic
    payload: activity
  ]
...
# XXX: log
Hub::sign = (message, secret) ->
  hmac = crypto.createHmac("sha1", secret)
  hmac.update message
  hmac.digest "hex"

Hub::postMessage = (targetUrl, message, signature, callback) ->
  parts = url.parse(targetUrl)
  options =
    host: parts.hostname
    port: parts.port
