...
var MAX_LEASE     = 60 * 60 * 24 * 365;
var MIN_LEASE     = 60 * 60 * 24 * 1;
var DEFAULT_LEASE = 60 * 60 * 24 * 7;

var Hub = function(server, db) {
    this.server = server;
    this.db = db;
};
...
Hub.prototype.ourTopic = function(topic) {
    var parts = url.parse(topic);

    return (parts.hostname === this.server &&
            parts.pathname === '/feed.json');
};
...
Hub.prototype.subscribe = function(params, cb) {
    ...
    this.validate(params, function(err) {
        ...
        if (err) {
            ...
            this.db.save('subscription', callback, sub, function(err, value) {
                if (err) {
...
Hub.prototype.unsubscribe = function(params, cb) {
    ...
    this.validate(params, function(err) {
        ...
        if (err) {
            ...
            this.db.del('subscription', callback, function(err) {
...
Hub.prototype.validate = function(params, cb) {
    ...
    this.verify(params, function(err) {
        var sub;
        if (err) {
            cb(err);
        } else {
...
Hub.prototype.distribute = function(activity, topic, callback) {
    ...
    this.db.search('subscription', {'topic': topic}, onSub, function(err) {
        callback(err);
    });
};
